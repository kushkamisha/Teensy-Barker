...
const processWebpage = (webpage, dataFolder) => new Promise((response, reject) => {
    ...
    request(webpage.url)
        ...
        .on('response', res => {
            ...
            if (!utils.saveFileFromUrl(res, dataFolder))
                if (webpage.url !== webpage.homeUrl) {
                    creatingPdf = true
                    // logger.info(`before: ${Date.now() % 10000} ${webpage.url}`)
        ...
        .on('complete', () => {
            // logger.info(`after: ${Date.now() % 10000} ${webpage.url} ${creatingPdf}`)
            // logger.info(webpage.toProcess.size)
            // logger.info(!creatingPdf && !webpage.toProcess.size)
            // Create child webpages if needed
            if (!creatingPdf && !webpage.toProcess.size) {
                logger.info('No pages to process response')
                response()
            }
            const pages = createChildWebpages(webpage)

            if (pages.length) {
                // logger.info('Promise.all response')
